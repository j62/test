  ...
  <tr bgcolor="#FFFFFF">
    <td>
		<table style="margin-bottom:8px;margin-top:8px;margin-left:20px;">
		  <form action="./do_product_cate_edit.php" method="post">
			 <?php
				$sql="select pc_name from ts_product_cate where pc_id='$pc_id'";
				$re=mysql_query($sql);
			 ...
			 <tr>
				 <td>分类名称:</td>
				 <td><input type="text" name="pc_name" value="<?=$ar['pc_name']?>"></td>
			   </tr>
			 <tr>
					...
					?>>
						...
						<?php
							foreach($arr as $key=>$vo){ ?>
								<option value="<?=$vo['pc_level'].'/'.$vo['pc_id']?>" <?=$pc_f_id==$vo['pc_id']?"selected":''?>>
